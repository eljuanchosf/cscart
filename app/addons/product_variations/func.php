...
function fn_product_variations_get_available_options($product_id, $lang_code = CART_LANGUAGE)
{
    ...
    $product_options = fn_get_product_options($product_id, $lang_code, true, true);
...
function fn_product_variations_delete_product_option_before_delete($option_id, $pid, $product_id, $product_link, &$can_continue)
{
    if ($product_id) {
        ...
        // we deleting linked option from the product
        $product_ids = $product_link;
    } else {
        ...
        if (is_array($variation_products)) {
            ...
            if ($parent_product_ids) {
                ...
                fn_set_notification(
                    'E',
                    __('error'),
                    __('product_variations.cannot_remove_options_that_used_for_variation',
                        array(
...
/**
 * Hook handler: Gets the options of a configurable product.
 */
function fn_product_variations_clone_product_option_post($from_product_id, $to_product_id, $from_global_option_id, $option_data, $change_options, $change_variants)
{
    Tygh::$app['session']['product_clone'] = isset(Tygh::$app['session']['product_clone']) ? Tygh::$app['session']['product_clone'] : array();

    $options = array();
    foreach ($change_options as $option_id_clone => $option_id) {
        $options[$option_id_clone] = array(
            'option_id' => $option_id,
            'variants' => $change_variants
        );

        Tygh::$app['session']['product_clone']['select_options'][$option_id] = $option_id;
    }

    Tygh::$app['session']['product_clone']['product_id'] = $to_product_id;
    Tygh::$app['session']['product_clone']['options'] = $options;
}
...
function fn_product_variations_clone_product_post($product_id, $pid, $orig_name, $new_name)
{
    ...
    if (!empty(Tygh::$app['session']['product_clone']['select_options'])) {
        $option_ids = array_values(Tygh::$app['session']['product_clone']['select_options']);
        $product_manager->changeProductTypeToConfigurable($pid, $option_ids);
    }
...
function fn_product_variations_clone_product_data($product_id, &$data, $is_cloning_allowed)
{
    ...
    $options = Tygh::$app['session']['product_clone']['options'];

    if ($data['product_type'] != ProductManager::PRODUCT_TYPE_VARIATION) {
        Tygh::$app['session']['product_clone'] = array();
    ...
    $data['parent_product_id'] = Tygh::$app['session']['product_clone']['product_id'];
    $data['variation_code'] = $product_manager->getVariationCode(Tygh::$app['session']['product_clone']['product_id'],
