    ...
    public function getReportData()
    {
        if ($this->report_data === null) {
            $this->report_data = array(
                ...
                'db' => array(
                    'engine'  => $this->_database_settings['database_backend'],
                    'version' => \Tygh::$app['db']->getServerVersion(),
                    'strict'  => (int) $this->isStrictMode($this->_database_settings['database_backend']),
                ),
    ...
    protected function getUpdatesServerUrl()
    {
        if ($this->updates_server_url === null) {
            ...
            foreach ($config_contents as $line) {
                if (strpos($line, "'updates_server' => ") !== false) {
