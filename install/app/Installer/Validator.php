...
class Validator extends Validators
{
    ...
    /**
     * Checks whether either ImageMagick or GD image resize lib is available on the server.
     * Notifies user that the ImageMagick lib is recommended if the GD lib is used.
     *
     * @return bool
     */
    public function isValidImageResizeLibraryUsed()
    {
        $has_gd = $has_imagick = false;
        ...
        if (!$result) {
            $app->setNotification(
                'E',
                $app->t('error'),
                $app->t('image_resize_lib_not_available'),
                true,
                'server_configuration'
            );
        } elseif ($has_gd && !$has_imagick) {
            $app->setNotification(
                'W',
                $app->t('warning'),
                $app->t('bad_resize_lib'),
                true,
                'server_configuration'
            );
        }
