...
<div class="table-responsive-wrapper">
    ...
    <tr>
        ...
        <th class="mobile-hide" width="15%"><a class="cm-ajax" href="{"`$c_url`&sort_by=list_price&sort_order=`$search.sort_order_rev`"|fn_url}" data-ca-target-id={$rev}>{__("list_price")} ({$currencies.$primary_currency.symbol nofilter}){if $search.sort_by == "list_price"}{$c_icon nofilter}{else}{$c_dummy nofilter}{/if}</a></th>
        {if $search.order_ids}
    ...
    <tr class="cm-row-status-{$product.status|lower} cm-longtap-target {$hide_inputs_if_shared_product}" data-ca-longtap-action="setCheckBox" data-ca-longtap-target="input.cm-item">
        ...
        <td class="{if $no_hide_input_if_shared_product}{$no_hide_input_if_shared_product}{/if}" data-th="{__("price")}">
        ...
        <td  class="mobile-hide" data-th="{__("list_price")}">
        ...
        {if $search.order_ids}
        <td data-th="{__("purchased_qty")}">{$product.purchased_qty}</td>
        <td data-th="{__("subtotal_sum")}">{$product.purchased_subtotal}</td>
        {/if}
...
{capture name="buttons"}
    {capture name="tools_list"}
		...
		{if $products}
            <li class="divider "></li>
            <li class="">{btn type="dialog" class="cm-process-items" text=__("edit_selected") target_id="content_select_fields_to_edit" form="manage_products_form"}</li>
            <li class="">{btn type="list" text=__("clone_selected") dispatch="dispatch[products.m_clone]" form="manage_products_form"}</li>
            <li class="">{btn type="list" text=__("export_selected") dispatch="dispatch[products.export_range]" form="manage_products_form"}</li>
            <li class="">{btn type="delete_selected" dispatch="dispatch[products.m_delete]" form="manage_products_form"}</li>
        {/if}
        {/hook}
    {/capture}
    {dropdown content=$smarty.capture.tools_list}
