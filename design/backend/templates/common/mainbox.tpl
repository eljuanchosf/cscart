    ...
    <div class="title pull-left">
        {if isset($title_start) && isset($title_end)}
            <h2 class="title__heading"
                title="{$title_alt|default:"`$title_start` `$title_end`"|strip_tags|strip}"
            >
                <span class="title__part-start mobile-hidden">{$title_start}: </span>
                <span class="title__part-end">{$title_end|strip_tags}<span>
            </h2>
        {else}
            <h2 class="title__heading" title="{$title_alt|default:$title|strip_tags|strip}">{$title|default:"&nbsp;"|sanitize_html nofilter}</h2>
        {/if}
