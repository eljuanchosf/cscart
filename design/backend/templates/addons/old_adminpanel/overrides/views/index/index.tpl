                    ...
                    <td>
                        <div class="dashboard-card">
                            ...
                            <div class="dashboard-card-content">
                                ...
                                <bdi>{$orders_stat.prev_orders|count}, {if $orders_stat.diff.orders_count > 0}+{/if}{$orders_stat.diff.orders_count}</bdi>
                            </div>
                        </div>
                    </td>
                {/if}
                {if !empty($orders_stat.orders_total)}
                    <td>
                        <div class="dashboard-card">
                            ...
                            <div class="dashboard-card-content">
                                <h3><a href="{"orders.manage?is_search=Y&period=C&time_from=`$time_from`&time_to=`$time_to`"|fn_url}">{include file="common/price.tpl" value=$orders_stat.orders_total.totally_paid}</a></h3><bdi>{include file="common/price.tpl" value=$orders_stat.prev_orders_total.totally_paid}, {if $orders_stat.orders_total.totally_paid > $orders_stat.prev_orders_total.totally_paid}+{/if}{$orders_stat.diff.sales nofilter}%</bdi>
                            </div>
                        </div>
                    </td>
                {/if}
                {if !empty($orders_stat.taxes)}
                    <td>
                        <div class="dashboard-card">
                            ...
                            <div class="dashboard-card-content">
                                <h3>{include file="common/price.tpl" value=$orders_stat.taxes.subtotal}</h3>
                                <bdi>{include file="common/price.tpl" value=$orders_stat.taxes.prev_subtotal}, {if $orders_stat.taxes.subtotal > $orders_stat.taxes.prev_subtotal}+{/if}{$orders_stat.taxes.diff nofilter}%</bdi>
                            </div>
                        </div>
                    </td>
                {/if}
                {if !empty($orders_stat.abandoned_cart_total)}
                    <td>
                        <div class="dashboard-card">
                            ...
                            <div class="dashboard-card-content">
                                <h3><a href="{"cart.cart_list"|fn_url}">{$orders_stat.abandoned_cart_total|default:0}</a></h3><bdi>{$orders_stat.prev_abandoned_cart_total|default:0}, {if $orders_stat.abandoned_cart_total > $orders_stat.prev_abandoned_cart_total}+{/if}{$orders_stat.diff.abandoned_carts nofilter}%</bdi>
                            </div>
                        </div>
                    </td>
                {/if}
                        ...
                        <div class="item">
                            {hook name="index:recent_activity"}
                                ...
                                {elseif $item.type == "orders" && "orders.details?order_id=`$item.content.id`"|fn_url|fn_check_view_permissions:"GET"}
                                    ...
                                    <a href="{"orders.details?order_id=`$item.content.id`"|fn_url}"><span>{__("order")}</span>&nbsp;<bdi>{$item.content.order}</bdi></a><br>
                                ...
                                {elseif $item.type == "categories" && "categories.update?category_id=`$item.content.id`"|fn_url|fn_check_view_permissions:"GET"}
                                    <a href="{"categories.update?category_id=`$item.content.id`"|fn_url}"><bdi>&lrm; {$item.content.category}</bdi></a><br>
                                {/if}

                                {hook name="index:recent_activity_item"}{/hook}

                                <span class="date"><bdi>{$item.timestamp|date_format:"`$settings.Appearance.date_format`, `$settings.Appearance.time_format`"}</bdi></span>
                            {/hook}
                        </div>
                    {/if}
                {/function}
